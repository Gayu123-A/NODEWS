...
const { createClient } = require('redis'); // for caching
...
// Connect to Redis (Required in Redis 4+)
(async () => {
...
// Middleware: Log requests using Winston
app.use((req, res, next) => {
    logger.info(`${req.method} ${req.url} - ${req.ip}`);
    next();
});

// Middleware to set the view engine, views directory and static files folder
...
app.get('/auth/google/callback',
    ...
    (req,res) => {
        logger.info(`User logged in: ${req.user.displayName}`);

        // Queue an email after successful login
        emailQueue.add({
            email: req.user.emails[0].value,
            subject: "Welcome!",
            message: `Hi ${req.user.displayName}, you have successfully logged in!`
        });
...
// Add Log Event for Dashboard Load
app.post("/api/log", (req, res) => {
    logger.info(`Client Event: ${req.body.event}`);
    res.sendStatus(200);
});
...
app.get('/weather', async (req, res) => {
    try {
        ...
        if (lat && lon) {
         ...
         if (cachedWeather) {
             logger.info(`Cache hit: Returning cached weather for ${cacheKey}`);
             return res.json(JSON.parse(cachedWeather));
         }

        // Fetch from OpenWeather API if not in cache
        url = lat && lon
        ...
        // Store in Redis Cache (Expire in 300 seconds = 5 minutes)
        await redisClient.setEx(cacheKey, 300, JSON.stringify(weatherInfo));

        logger.info(`Cache miss: Fetching weather from API for ${cacheKey}`);
...
app.get('/logout', (req, res) => {
    logger.info(`User logged out: ${req.user.displayName}`);
    req.logout((err) => {
